...
final adminOrdersProvider = FutureProvider.autoDispose<List<Map<String, dynamic>>>((ref) async {
  ...
  final r = await dio.get('/admin/orders/');
  final data = r.data;
...
class AdminOrdersScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: async.when(
        data: (rows) {
          if (rows.isEmpty) return const Center(child: Text('No orders.'));
                ...
                return ListTile(
                  ...
                  onTap: () => Navigator.of(context).push<void>(
                    MaterialPageRoute<void>(
                      builder: (_) => OrderDetailScreen(orderId: o['id'] as int, mode: OrderDetailMode.admin),
                    ),
                  ),
                );
              },

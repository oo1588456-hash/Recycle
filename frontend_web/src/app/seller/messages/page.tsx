...
export default function SellerMessagesPage() {
    ...
    async function boot() {
      ...
      if (restricted) {
        try {
          ...
          const fullName = (r.data as { full_name?: string }).full_name;
          setPeer(id);
          setProduct(null);
          setActiveLabel(fullName || "ReCycle admin");
          await loadMsgs(id, null);
        } catch {
          /* ignore */
        }
      }
  ...
  }, [user?.role, user?.seller_account_status, loadConvs, loadMsgs]);
        ...
        {!peer ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-2 p-8 text-center text-recycle-muted">
            ...
            <p className="max-w-sm text-xs">
              Use <strong>Platform</strong> to message marketplace staff (approvals, account help). Buyer threads about
              your listings appear here when buyers message you.
            </p>
          </div>
        ) : (
          <>
